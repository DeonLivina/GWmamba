...
# =====================================================================
# 1. CORNER PLOT GENERATION (standalone function, called AFTER reloading
# the best checkpoint -- see main() -- so plots reflect the best epoch,
# not the last one)
# =====================================================================
@torch.no_grad()
def generate_corner_plots(pl_module, data_loader, label_names, detector_names, out_dir="plots_before_model"):
    ...
    g_strain = sns.pairplot(
        strain_df, hue="Event Type", corner=True,
        palette=CLASS_COLOR_MAP, plot_kws={"alpha": 0.5, "s": 10}
    )
    g_strain.fig.suptitle("before_model: Strain Projection Space (bypass head, by Event Type)", y=1.02, fontsize=14)
    ...
    g_witness = sns.pairplot(
        witness_df, hue="Detector", corner=True,
        palette="Dark2", plot_kws={"alpha": 0.5, "s": 10}
    )
    g_witness.fig.suptitle("before_model: Witness Projection Space (bypass head, by Detector)", y=1.02, fontsize=14)
...
# 2. LIGHTNING MODULE
# =====================================================================
class BeforeModelLitModule(L.LightningModule):
    """Total loss = ce_weight * CE(logits, y_type)
                   + supcon_weight * (SupCon(strain_z, y_type) + SupCon(witness_z, y_detector))

    logits come from the FUSION path (shared Mamba over 1+N streams, pooled,
    concatenated, classified) -- NOT from strain_z/witness_z. strain_z and
    witness_z come from the separate BYPASS attention-pool heads and are
    used only for the two SupCon losses.
    """
    ...
    def _step(self, batch, stage):
        ...
        loss = self.ce_weight * ce_loss + self.supcon_weight * supcon_loss
...
def main():
    ...
    print(f"\n--- INITIALIZING before_model (n_wit={n_wit}) ---")
    model = BeforeModel(
        d_model=32,
        mamba_layers=4,
        proj_dim=8,
    ...
    lit_model = BeforeModelLitModule(model, lr=1e-3, temperature=0.1, supcon_weight=1.0, ce_weight=1.0)

    ckpt_cb = ModelCheckpoint(
        dirpath="checkpoints_before_model", monitor="val_acc", mode="max",
    ...
    trainer = L.Trainer(
        ...
        devices=1,
        callbacks=callbacks,
        logger=WandbLogger(project="ligo-before-model", name="before_model"),
    )

    trainer.fit(lit_model, train_loader, val_loader)

    print(f"\nLoading best checkpoint (by val_acc): {ckpt_cb.best_model_path}")
    best_lit_model = BeforeModelLitModule.load_from_checkpoint(
