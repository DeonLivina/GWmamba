...
same one inject_signal.py uses), rather than background_triggers.csv:
  ...
  1. Shuffles full_data/<DETECTOR>/whitened_background_full.h5's GPS list and
     ...
     background file only stores the final whitened kernel, not the raw
     PSD-context data needed to redo whitening after injection).
  2. Injects a synthetic SineGaussian blip into that raw strain only (real
     witness is whitened as-is, no injection), and keeps going until
     N_TARGET blip examples have been built or the background pool is
     exhausted.
  3. Removes exactly the background-file rows that were successfully
     converted into blip examples, so the same physical window doesn't end
     up counted as both "background" and this class (mirrors
     inject_signal.py's handling of "background" vs "signal").
...
import h5py
import numpy as np
import pandas as pd  # noqa: F401  (kept for parity with sibling scripts; unused here)
import torch
...
# big_model/utils/whitened_gaussians.py -> big_model/ -> dataset/
...
from witness import bandlimit, _rms_normalize
...
output_file = OUTPUT_DIR / "whitened_gaussians.h5"
...
def whiten_witness_block(block, rate, target_rate, transforms, psd_len_native, length):
    """Real data only -- no injection (witness is never touched by the
    strain-only blip)."""
...
pbar = tqdm(total=N_TARGET, desc="Injecting strain-only blips")
...
for win_start, orig_gps in zip(candidates, shuffled_gps):
    ...
    try:
        ...
        # -----------------------
        # Build the blip: strain only, no witness coupling. Bandlimited to
        # the detector band (matches dataset/injections.py's
        # `_inject_glitch`), RMS-normalised, time-jittered, then
        # rescaled/reweighted to a target strain SNR drawn from the
        # config's own glitch SNR distribution -- unlike the CBC "signal"
        # class, this does NOT bootstrap SNRs from an existing glitch
        # population file.
        # -----------------------
        ...
        blip = bandlimit(blip, TARGET_RATE, f_min, f_max)
        strain_g = _rms_normalize(blip).unsqueeze(1)  # (1, 1, kernel_size)

        strain_g, _ = _time_jitter(strain_g, sig_cfg, TARGET_RATE, device)

        raw_inj = raw[..., psd_size:]

        strain_g = strain_g * raw_inj.std(dim=-1, keepdim=True)
        target_strain_snr = _sample_target_snr(config.glitch.snr.strain, 1, device)
        strain_g = reweight_snrs(strain_g, target_strain_snr, psd_i, TARGET_RATE, highpass=f_min)
        snr = compute_network_snr(strain_g, psd_i, TARGET_RATE, highpass=f_min)
    ...
    # Whiten only the pad+kernel+pad slice -- NOT concatenated with the PSD
    # region (Whiten() crops fduration*sample_rate off the edges of whatever
    # it's given, not the full psd_size region).
    injected_segment = _inject_center(raw_inj.clone(), strain_g, kernel_size, pad)
    ...
    # -----------------------
    # Witness: real data only, whitened, no injection.
    # -----------------------
    w_samples = []
    ok = True
    for channel in WITNESS_CHANNELS:
...
print("\nFinished")
print("Strain-only blip examples built:", count)
...
if count == 0:
    raise RuntimeError("No strain-only blip examples produced.")
...
# -----------------------
# Remove the consumed windows from the background file, exactly like
# inject_signal.py, so the same physical window can't also be drawn later
# for "background" or any other class sourced from this pool.
# -----------------------
